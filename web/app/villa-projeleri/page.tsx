...
/* Villa projeleri vitrini — bkz. `app/konut-projeleri/page.tsx`. */
export const revalidate = 3600;

export async function generateMetadata(): Promise<Metadata> {
  const v = TIP_VITRINLERI.VILLA;
  const list = await getProjelerByTip('VILLA');
  const enDusuk = list.length ? Math.min(...list.map((p) => p.fiyatMin)) : 0;

  return meta({
    baslik: `${v.h1} — KonutProjeleri`,
    aciklama: list.length
      ? `${list.length} villa projesi.${enDusuk ? ` ${TLkisa(enDusuk)}'den başlayan fiyatlar.` : ''} `
        + v.metaOzet
      : v.metaOzet,
    yol: `/${v.slug}`,
...
export default function VillaProjeleriSayfasi() {
  return <TipVitrini tip="VILLA" />;
}
