  ...
  return (
    <main className="kp-wrap" style={{ paddingBlock: 'var(--s-5)' }}>
      ...
      <header style={{ marginBottom: 'var(--s-4)' }}>
        ...
        <div className="kp-row" style={{ marginTop: 'var(--s-3)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 330px', gap: 'var(--s-4)', alignItems: 'start' }}>
        <div className="kp-stack">
          ...
          {/* ── Sayfanın omurgası ── */}
          <section className="kp-card" style={{ padding: 'var(--s-5)' }} id="daireler">
                ...
                <thead>
                  <tr>
                    {['Tip', 'Net / brüt', 'Başlangıç fiyatı', 'm² birim', 'Aylık taksit', 'Kalan', ''].map((h) => (
                      <th key={h} className="kp-label" style={{ textAlign: 'left', padding: '0 10px 9px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {p.daire_tipleri.map((d) => {
                    ...
                    return (
                      <tr key={d.tip} style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: 11 }}>
                          {d.kalan_adet != null ? (
                            <span className="kp-row" style={{ gap: 7 }}>
                              <span
                                aria-hidden
                                style={{
          ...
          {/* Ödeme senaryoları */}
          {odeme && anaTip && (
            <section className="kp-card" style={{ padding: 'var(--s-5)' }} id="odeme">
              <h2 className="kp-h2">Ödeme planı · {anaTip.tip}</h2>
          ...
          {/* Konum ve çevre */}
          {p.cevre.length > 0 && (
            <section className="kp-card" style={{ padding: 'var(--s-5)' }} id="konum">
              <h2 className="kp-h2">Konum ve çevre</h2>
              <dl style={{ display: 'grid', gap: 0, margin: 0 }}>
                {p.cevre.map((c) => (
                  ...
                  >
                    ...
                    <dd style={{ margin: '0 0 0 auto', fontWeight: 750 }} className="tabular">
          ...
          {Object.keys(p.ozellikler ?? {}).length > 0 && (
            <section className="kp-card" style={{ padding: 'var(--s-5)' }}>
              <h2 className="kp-h2">Proje özellikleri</h2>
              <p className="kp-label" style={{ marginBottom: 'var(--s-3)' }}>Firmanın beyanına dayanır</p>
          ...
          {/* Firma karnesi özeti */}
          <section className="kp-card" style={{ padding: 'var(--s-5)' }} id="firma">
            <h2 className="kp-h2">Firma</h2>
            <div className="kp-row" style={{ gap: 'var(--s-4)' }}>
              <div>
                <Link href={`/firmalar/${p.firma_slug}`} style={{ fontSize: 16, fontWeight: 750 }}>
                  {p.firma_ad}
                </Link>
                <p className="kp-lead" style={{ marginTop: 4 }}>
          ...
          {p.benzer.length > 0 && (
            <section className="kp-card" style={{ padding: 'var(--s-5)' }}>
              <h2 className="kp-h2">Benzer projeler</h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 'var(--s-3)' }}>
                {p.benzer.map((b) => (
                  <Link key={b.slug} href={`/${b.il}/${b.ilce}/${b.slug}`} style={{ display: 'block' }}>
                    ...
                    <span style={{ display: 'block', fontSize: 13, fontWeight: 800, marginTop: 4 }} className="tabular">
        ...
        <aside className="kp-stack" style={{ position: 'sticky', top: 16 }}>
          <div className="kp-card" style={{ padding: 'var(--s-5)' }}>
            {anaTip && minFiyat ? (
              <>
                <span className="kp-label">{anaTip.tip} fiyatları</span>
                <span className="kp-num" style={{ display: 'block', fontSize: 25 }}>
                ...
                {odeme && (
                  <div style={{ background: 'var(--brand-soft)', borderRadius: 'var(--r-block)', padding: 'var(--s-3)', marginTop: 'var(--s-3)' }}>
                    ...
                    <b className="tabular" style={{ fontSize: 17, color: 'var(--brand-strong)' }}>
                      {para(odeme.senet)} / ay
                    </b>
                  </div>
                )}
          ...
          <form className="kp-card" style={{ padding: 'var(--s-5)' }} action="/api/talep" method="post">
            <h2 className="kp-h3" style={{ marginBottom: 4 }}>Fiyat listesi ve ödeme planı isteyin</h2>
          ...
          <div className="kp-card" style={{ padding: 'var(--s-4)' }}>
            ...
            <p className="kp-lead" style={{ fontSize: 11.5 }}>
              Son 7 günde <b className="tabular">{p.goruntulenme.toLocaleString('tr-TR')}</b> görüntüleme
...
function Ozet({ baslik, deger, alt }: { baslik: string; deger: string | null; alt?: string | null }) {
  ...
  return (
    <div style={{ background: 'var(--surface-card)', padding: 'var(--s-4)' }}>
      ...
      <dd style={{ margin: 0, fontSize: 15, fontWeight: 800, letterSpacing: '-0.03em' }} className="tabular">
        {deger}
...
function Senaryo({
  ...
  return (
    <div style={{
      ...
      <div className="kp-row" style={{ gap: 6, marginBottom: 4 }}>
        <b style={{ fontSize: 13 }}>{baslik}</b>
        {rozet && <Pill durum="success">{rozet}</Pill>}
      </div>
      <span className="kp-num tabular" style={{ fontSize: 19 }}>{tutar}</span>
